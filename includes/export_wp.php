 ...
 */
function t2export_wp( $args = array() ) {
	global $wpdb, $post;

	$defaults = array( 'content' => 'post', 'author' => false, 'category' => false,
		'start_date' => false, 'end_date' => false, 'status' => false,
		'post_types' => array('posts'), 'page_ids' => array(),
	);
	$args = wp_parse_args( $args, $defaults );

	do_action( 't2export_wp', $args );
	...
	*/
	...
	$query_ids = "SELECT ID FROM {$wpdb->posts} $join WHERE $where";
	$post_ids = $wpdb->get_col( $query_ids );
	$attach_ids = $wpdb->get_col( "SELECT ID FROM {$wpdb->posts} WHERE {$wpdb->posts}.post_type='attachment' AND {$wpdb->posts}.post_parent IN ($query_ids)" );
	$post_ids = array_merge($attach_ids, $post_ids); // add attachments
	$post_ids = array_merge($post_ids, $args['page_ids']); //add selected page ids
